...
class BackgroundTasksMixin:
    ...
    def _run_audit_thread(
        ...
        try:
            ...
            if memory is None:
                memory = {"turn": 0, "mu": {}}  # Use empty dict for modern semantic format
            else:
                current_mu = memory.get("mu")
                # Handle both formats:
                # - Modern (dict): No dimension check needed, Spirit maps by value name
                # - Legacy (list/array): Check length matches current value count
                if current_mu is None:
                    memory = {"turn": 0, "mu": {}}
                elif isinstance(current_mu, (list, np.ndarray)) and len(current_mu) != required_dim:
                    self.log.warning(f"Spirit dimension mismatch (legacy format). Resetting Spirit Memory.")
                    memory = {"turn": 0, "mu": {}}
                # Dict format is always compatible - Spirit will map by normalized value names
