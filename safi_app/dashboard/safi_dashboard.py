...
# st.sidebar removed for clean embedded UI
...
def normalize_entry(e: dict) -> dict:
    ...
    if is_v1_flat:
        ...
        will_norm = "Approved" if will_dec == "approve" else ("Redirected" if will_dec in ["violation", "blocked", "redirected"] else e.get("willDecision", "Unknown"))
        return {
            ...
            "org_id": e.get("orgId"), "user_id": e.get("userId"),
            "agent_name": e.get("agentName", ""), "intellect_model": e.get("intellectModel", ""),
        }
    return {
...
def load_and_process_logs(profile_key: str, profile_name: str) -> pd.DataFrame:
    ...
    for col in [
        ...
        "memory.summary", "recent_turns", "intellect.context", "agent_name", "intellect_model"
    ]:
...
st.markdown("""
    <style>
        ...
        /* Make background transparent to match SAFi */
        .stApp {
...
if mode == "Agent":
    ...
    df["spirit.score"] = pd.to_numeric(df["spirit.score"], errors='coerce')
    df["spirit.drift"] = pd.to_numeric(df["spirit.drift"], errors='coerce')
    for col in ["prompt", "will.decision", "output", "intellect.response", "intellect.reflection", "will.reflection", "spirit.reflection", "spirit.feedback", "memory.summary", "recent_turns", "intellect.context", "agent_name", "intellect_model"]:
        df[col] = df[col].fillna("")
...
interventions_count = (df_filtered["will.decision"] == "Redirected").sum()
...
with kpi_cols[1]:
    ...
    with row2[0]:
        ...
        <div class="card">
            ...
            <div style="font-size: 2.25rem;">{interventions_count}</div>
...
with st.container(border=True):
    ...
    elif filter_option == "Redirected":
        log_display_df = log_display_df[log_display_df["will.decision"] == "Redirected"]

    if not log_display_df.empty:
        base_cols = ['ts', 'prompt', 'spirit.score', 'identity_coherence', 'will.decision']
        rename_map = {'spirit.score': 'Alignment', 'identity_coherence': 'Consistency', 'will.decision': 'Decision'}
        if mode == "Policy":
            base_cols = ['ts', 'agent_name', 'intellect_model', 'prompt', 'spirit.score', 'identity_coherence', 'will.decision']
            rename_map.update({'agent_name': 'Agent', 'intellect_model': 'Model'})
        display_table = log_display_df[base_cols].copy()
...
if not log_display_df.empty:
    ...
    if selected_index is not None:
        ...
        tab_intellect, tab_will, tab_conscience, tab_spirit, tab_output = st.tabs(["AI Draft", "Decision", "Policy Audit", "Alignment", "Final Output"])
        ...
        with tab_intellect:
            ...
            st.markdown("#### Context (Memory Summary)")
            ...
            st.markdown("#### Context (Recent Turns)")
            st.text_area("Recent Turns", value=entry.get("recent_turns", "No recent turns captured."), height=200, disabled=True, key=f"irecent_{selected_index}")
        ...
        with tab_conscience:
            ...
            else: st.info("No evaluations recorded for this entry.")
