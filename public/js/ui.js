...
// Object to hold references to all key DOM elements
...
/**
 * Toggles the theme icon visibility based on the current theme.
 */
...
export function displayMessage(sender, text, date = new Date(), ledger = null, whyHandler) {
    elements.emptyState.classList.add('hidden');
    maybeInsertDayDivider(date);
    const container = document.createElement('div');
    const html = DOMPurify.sanitize(marked.parse(text ?? ''));
    
    // --- CHANGE: Added an inline style attribute for direct styling ---
    ...
    if (sender === 'user') {
        ...
        let whyButtonHtml = ledger && ledger.length > 0 ? `<button class="why-btn text-xs text-green-600 dark:text-green-500 font-semibold hover:underline mt-2">Why this answer?</button>` : '';
        container.innerHTML = `
        ...
        container.querySelector('.why-btn')?.addEventListener('click', () => whyHandler(ledger));
    }
...
export function showModal(type, ledger) {
    if (type === 'conscience') {
        elements.conscienceDetails.innerHTML = '';
        ledger.forEach(item => {
            const score = item.score;
            const color = score > 0 ? 'text-green-500' : score < 0 ? 'text-red-500' : 'text-neutral-500';
            const icon = score > 0 ? '▲ Upholds' : score < 0 ? '▼ Challenges' : '● Neutral';
            const detailEl = document.createElement('div');
            detailEl.className = 'p-3 bg-neutral-100 dark:bg-neutral-800 rounded-lg';
            detailEl.innerHTML = `<p class="font-semibold ${color}">${icon} ${item.value}</p><p class="text-sm text-neutral-600 dark:text-neutral-400 mt-1">${item.reason}</p>`;
            elements.conscienceDetails.appendChild(detailEl);
        });
